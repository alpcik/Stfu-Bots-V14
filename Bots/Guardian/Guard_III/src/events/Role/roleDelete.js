...
const newRole = await role.guild.roles.create({
  ...
  let kanstfuermVeri = data.channelOverwrites.filter(e => client.guilds.cache.get(stfucum.GuildID).channels.cache.get(e.id))
  if (kanstfuermVeri) kanstfuermVeri.forEach((perm, index) => {
    ...
    setTimeout(() => {
      ...
      kanal.permissionOverwrites.create(newRole, yeniKanstfuermVeri).catch(console.error);
    }, index * 5000);
  });
