...
.setDescription(`
${entry.executor} üyesi **${stfucum.Guard.Limit.RoleCreate}** limitinden fazla rol açmayı denediği için jaile attım.
─────────────────────
Yetkili: (${entry.executor} - \`${entry.executor.id}\`)
─────────────────────
Tarih: \`${moment(Date.now()).format("LLL")}\``)

let stfuGuardLog = await role.guild.channels.cache.find(x => x.name == "guard_log").send({ embeds: [stfu], components: [row] });
...
.setThumbnail(entry.executor.avatarURL({ dynamic: true }))
.setDescription(`
${entry.executor} üyesinin geriye kalan rol açma limiti: **${roleCreateLimit[entry.executor.id]}/${stfucum.Guard.Limit.RoleCreate}**.
─────────────────────
Yetkili: (${entry.executor} - \`${entry.executor.id}\`)
─────────────────────
Tarih: \`${moment(Date.now()).format("LLL")}\``)

return role.guild.channels.cache.find(x => x.name == "guard_log").send({ embeds: [stfu] });
};
