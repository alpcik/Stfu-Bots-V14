...
let küfürler = ["stfucumoc","stfucumoç","stfucumamk","stfucumaq","0r0spuc0cu","4n4n1 sk3r1m","p1c","@n@nı skrm","evladi","orsb","orsbcogu","amnskm","anaskm","oc",
"a\u011fz\u0131na s\u0131\u00e7ay\u0131m","seks","sex","stfucums\u0131z","amar\u0131m","ambiti","am biti","amc\u0131\u011f\u0131","amc\u0131\u011f\u0131n",
...
"bac\u0131n\u0131","bac\u0131n\u0131n","bacini","bacn","bacndan","bacy","b\u0131z\u0131r","bosalmak","bo\u015falmak","daltassak","dalyarak","dalstfuux","dassagi",
...
"yarra\u011f\u0131m","yarra\u011f\u0131m\u0131","yarraimin","stfuux","yarram","yarramin","yarraminba\u015f\u0131","yarramn","yarran","yarrana","yarrrak","yav\u015f","yav\u015fak",
...
module.exports = async (message) => {
    ...
    if (stfucum.Guard.kufurEngel && küfürler.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(message.content))) return sendChat(message, "Umarım küfür etmedin yoksa sikerim belanı güzel kardeşim küfür senin o güzel diline hiç yakışıyormu ayıp edep erkan öğrenin az")
    ...
    if ((stfucum.Guard.capsEngel) && (Caps / message.content.length) >= 0.7) return sendChat(message, "Caps-Lock kullanmayı lütfen azaltalım")

    if (stfucum.Guard.reklamEngel && message.content.match(inviteEngel)) {
    ...
    if(stfucum.Guard.reklamEngel && reklamlar.some(word => message.content.toLowerCase().includes(word))) return sendChat(message, "Lütfen reklam yapmayı bırakınız")
}
...
client.on("messageCreate", async (message) => {
  if(message.webhookID || message.author.bot || message.channel.type === ChannelType.DM) return;
  if (await client.checkPermission(client, message.author.id, "full") || await client.checkPermission(client, message.author.id, "chatguard")) return;
  if (stfucum.Guard.spamEngel == false) return;
...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    ...
    if (stfucum.Guard.kufurEngel && küfürler.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(newMessage.content))) newMessage.delete().catch(err => {});
    ...
    if (stfucum.Guard.reklamEngel && newMessage.content.match(inviteEngel)) {
    ...
    if(stfucum.Guard.kufurEngel && reklamlar.some(word => newMessage.content.toLowerCase().includes(word))) return newMessage.delete().catch(err => {})
});
    ...
    async function sendChat(message, content) {
        if ((Number(getLimit.get(`${message.member.id}`))) == 3) {
            ...
            client.guilds.cache.get(stfucum.GuildID).members.cache.get(message.member.id).roles.add(setup.chatMute)
