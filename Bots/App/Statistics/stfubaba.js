...
const stfucumdb = (global.stfusetupxd = new Database("../src/configs/sunucuayar.json"));
...
client
  .login(stfucum.Bots.Stats_Token)
...
client.checkLevel = async function (userID, guildID, type) {
  if (stfucum.Bots.LevelSystem == false) return;
  let sunucu = client.guilds.cache.get(guildID);
  if (!sunucu) return;
  let kontrol = await stats.findOne({ userID: userID, guildID: guildID });
  if (!kontrol) return;
  const channel = client.channels.cache.find(x => x.name == "level_log");
  let member = client.guilds.cache.get(stfucum.GuildID).members.cache.get(userID)
