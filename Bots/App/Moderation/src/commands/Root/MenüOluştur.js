                        ...
                        if(i.customId == "kur") {
                            ...
                            if(kurulcak) {
                                ...
                                message.channel.send({content: `${kurulcak.Text}`, components: [listMenu]}).then(async (oluşturuldu) => {
                                    ...
                                    collector.on('collect', async (i) => {
                                        const member = await client.guilds.cache.get(stfucum.GuildID).members.fetch(i.user.id)
                                        if (!member) return;
                                ...
                                i.reply({content: `Başarıyla **${kurulcak.Name}** isimli rol seçim menüsü kuruldu.`, ephemeral: true})
                                msg.delete().catch(err => {})
                                message.react("✅")
                            }
                        }
...
Yeni oluşturulmakta olan rol seçim menünüze bir isim belirleyin.`)]}).then(async (isimbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        ...
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return isimbelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`:x: Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
.then(async (açıklamabelirleme) => { 
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return açıklamabelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`:x: Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
Yeni oluşturulmakta olan rol seçim menünüzde listelenecek rolleri belirtin.`).setFooter({ text:`En az 3 tane, en fazla 25 tane rol ekleyebilirsiniz.`})]}).then(async (rolbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
           return rolbelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`:x: Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
Başarıyla **${rolSeçim.Name}** isimli rol seçim menüsü \`${moment(Date.now()).format("LLL")}\` tarihinde oluşturuldu.`)]}).then(async (oluşturuldu) => {
