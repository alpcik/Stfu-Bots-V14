...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    await message.channel.send({ content: `:tada: **${guild.name}** Sunucusunun Özel Oda sistemi!\n\nAşağıdaki butonlar yardımıyla yeni bir özel oda oluşturabilirsin ya da parola ile katılabilirsin.\n**Panele Erişim:** [\` Sesli Sohbet Kanalınızın Sohbet Kısmından Odanızın Kontrol Paneline Ulaşabilirsiniz! \`]`, ephemeral: false, components: [row]});
  },
};
...
client.on('interactionCreate', async interaction => {
    if(interaction.customId === "create") {
        ...
        const xd = await client.guilds.cache.get(stfucum.GuildID).channels.cache.get(data?.ID)

        if (data?.ID && xd) return interaction.reply({ embed: `<@${interaction.user.id}> zaten özel odanız var.`, ephemeral: true })
...
const Channel = Guild?.channels.cache.get(room?.ID);
    ...
    return }
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(stfucum.GuildID, { Connect: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(stfucum.GuildID, { Connect: true });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(stfucum.GuildID, { ViewChannel: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(stfucum.GuildID, { ViewChannel: true });
        ...
        let msg = await interaction.channel.send({ content: `<@${rooms.Owner}>`, embeds: [prostfu], components: [pro] })
        ...
        if (button.customId === "onays") {
            ...
            .setAuthor({ name: client.guilds.cache.get(stfucum.GuildID).name, iconURL: client.guilds.cache.get(stfucum.GuildID).iconURL({ dynamic: true, size: 2048 })})
    ...
    collector.on('end', i => {
       msg.delete();
        ...
        await interaction.reply({ content: `Lütfen kanalınıza girmesini istediğiniz üye(leri) etiketleyiniz. \`Örn: @stfu\``, ephemeral: true })
       ...
       let yarak = await interaction.channel.send({ content: `Odaya erişimini kaldırmak istediğiniz üyeleri listeden seçiniz.`, components: [row]});
            ...
            yarak.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000 }).then((x) => {
                x.values.map(async (v) => {
                    ...
                    let memb = client.guilds.cache.get(stfucum.GuildID).members.cache.get(v)
                    if (memb && memb.voice && memb.voice.channel) {
                        memb.voice.disconnect();
                      }
                });
                if (yarak) yarak.delete();
                interaction.channel.send({ content: `Başarılı bir şekilde <@${x.values}> adlı üyenin odanıza erişim izni silindi.` });
                });
        ...
        await interaction.reply({ content: `Kanal sahipliğini aktarmak istediğiniz üyeyi etiketleyiniz. \`Örn: @stfu\``, ephemeral: true })
        interaction.channel.awaitMessages({ filter, max: 1 }).then(async function (collected) {
...
client.on("ready", async () => {
    const sunucu = client.guilds.cache.get(stfucum.GuildID);
