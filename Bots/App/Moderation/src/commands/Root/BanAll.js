...
module.exports = {
  conf: {
    aliases: [],
    name: "topluban",
    help: "topluban <@stfu/ID> <stfuingo/ID>",
    category: "sahip",
    owner: true,
  },
  ...
  run: async (client, message, args) => {
    ...
    const stfucuk = await message.reply({ content: 
        `${members
            .map((member, idx) => `**${idx + 1}. ${member.toString()}**`)
            .join("\n")}\nBu üyeleri banlamak istiyor musun?` })

    await stfucuk.react(green);

    const filter = (reaction, user) => {
        return reaction.emoji.name === 'green' && user.id === message.author.id;
    };
    const collector = stfucuk.createReactionCollector({ filter, time: 10000 });

    collector.on("collect", async () => {
        await stfucuk.edit({ content: `${members.length} adet kullanıcı başarıyla yasaklandı.`})
    ...
    collector.on("end", (_, reason) => {
        if (reason === "time")
            stfucuk.edit({ content: `10 saniye geçtiği için işlem iptal edildi.`})
    });
