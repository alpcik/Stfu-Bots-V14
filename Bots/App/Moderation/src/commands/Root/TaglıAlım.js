...
module.exports = {
  ...
  run: async (client, message, args) => {  
    ...
    let data = await registerData.findOne({ guildID: stfucum.GuildID })
    if(!data) new registerData({guildID: stfucum.GuildID, tagMode: false}).save();
  ...
  collector.on("collect", async (button) => {
    ...
    if (button.customId === "ac") {
      ...
      if(data && data.SesMod === taglialim) return msg.edit({ content: `Zaten ses modu \` Taglı Alım \` olarak ayarlı`, components: [] });
        await Database.findOneAndUpdate({ guildID: stfucum.GuildID }, {SesMod: taglialim}, { upsert: true })
