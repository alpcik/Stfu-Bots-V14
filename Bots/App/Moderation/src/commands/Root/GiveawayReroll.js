...
module.exports = {
  ...
  run: async (client, message, args) => {
    if (
      ...
      if (isNaN(mesaj)) return message.reply({ content: `:x: Lütfen komutu doğru kullanın! \`.greroll [Mesaj-ID]\`` })
      let data = await giveaway.findOne({ messageID: mesaj });
      if (!data) return message.reply({ content: `:x: Mesaj ID'sinde datada veri bulunamadı.` })
...
.setDescription(`
:tada: Çekiliş kazananı yeniden seçildi!
