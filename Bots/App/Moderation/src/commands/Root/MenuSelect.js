...
const { stfunitro, stfunetflix, stfuspotify, stfuexxen, stfublutv, stfuyoutube, star } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client, message, args, durum, kanal) => {
    ...
    let stfu = await message.channel.send({
        "embeds": [embed],
          "components":[{
            "type":1,
            "components":[
        ...
        let collector = await stfu.createMessageComponentCollector({ filter,  time: 30000 })
        
        collector.on("collect", async (button) => {
        
            if (button.customId === "hepsi") {
            await stfu.delete({ timeout: 1500 });
    ...
    \` ⦁ \` Eğer \`@Çekiliş Katılımcısı\` Rolünü alırsanız sunucumuzda sıkça vereceğimiz **Nitro, Netflix, Spotify Premium** ve daha nice ödüllerin bulunduğu çekilişlerden haberdar olabilirsiniz. 
    ...
    **NOT:** \`Kayıtlı, kayıtsız olarak hepiniz bu kanalı görebilmektesiniz. Sunucumuz da everyone veya here atılmayacağından dolayı kesinlikle rollerinizi almayı unutmayın.\``,
            ...
            message.channel.send({ content: `:star2: Aşağıda ki menüden **Burç** rollerinden dilediğinizi alabilirsiniz.`,
                    "components": [{
                        "type": 1, "components": [{
            ...
            message.channel.send({ content: `:star2: Aşağıda ki menüden **Oyun** rollerinden dilediğinizi alabilirsiniz.`,
                    "components": [{
                        "type": 1, "components": [{
            ...
            message.channel.send({ content: `:star2: Aşağıda ki menüden **Renk** rollerinden dilediğinizi alabilirsiniz.`,
                    "components": [{
                        "type": 1, "components": [{
            ...
            message.channel.send({ content: `:star2: Aşağıda ki menüden **İlişki** rollerinden dilediğinizi alabilirsiniz.`,
                    "components": [{
                        "type": 1, "components": [{
    ...
    \` ⦁ \` Eğer \`@Çekiliş Katılımcısı\` Rolünü alırsanız sunucumuzda sıkça vereceğimiz ${stfunitro} , ${stfuspotify} , ${stfuyoutube} , ${stfunetflix} , ${stfuexxen} ve daha nice ödüllerin bulunduğu çekilişlerden haberdar olabilirsiniz. 
    ...
    **NOT:** \`Kayıtlı, kayıtsız olarak hepiniz bu kanalı görebilmektesiniz. Sunucumuz da everyone veya here atılmayacağından dolayı kesinlikle rollerinizi almayı unutmayın.\``,
    ...
    const etkinlik = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.etkinlik))
    const cekilis = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.cekilis))
    ...
    const koç = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.koç))
    const boğa = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.boğa))
    const ikizler = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.ikizler))
    const yengeç = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.yengeç))
    const aslan = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.aslan))
    const başak = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.başak))
    const terazi = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.terazi))
    const akrep = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.akrep))
    const yay = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.yay))
    const oğlak = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.oğlak))
    const kova = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.kova))
    const balık = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.balık))
    ...
    const csgo = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.csgo))
    const lol = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.lol))
    const valorant = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.valorant))
    const gta5 = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.gta5))
    const pubg = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.pubg))
    const fortnite = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.fortnite))
    ...
    const kirmizi = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.kirmizi))
    const turuncu = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.turuncu))
    const mor = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.mor))
    const pembe = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.pembe))
    const yesil = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.yesil))
    ...
    const sevgili = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.İlişkiler.couple))
    const yalnız = await client.guilds.cache.get(stfucum.GuildID).roles.cache.find(x => x.name.includes(ayar.İlişkiler.alone))
