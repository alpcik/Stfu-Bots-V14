...
module.exports = {
  ...
  collector.on("collect", async (button) => {
    ...
    if (button.customId === "mute") {
      ...
      if (stfucum.Bots.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından susturmanız kaldırıldı!`}).catch(() => {});
      ...
      await msg.edit({ embeds: [stfu], components: [row] });
    ...
    if (button.customId === "vmute") {
      ...
      if (stfucum.Bots.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından **sesli kanallarda** olan susturmanız kaldırıldı!`}).catch(() => {});
