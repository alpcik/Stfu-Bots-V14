...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.react("✅")
    message.reply({ content:`:white_check_mark: \`(${ban.user.username.replace(/\`/g, "")} - ${ban.user.id})\` adlı üyenin banı ${message.author} tarafından kaldırıldı!`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (stfucum.Bots.dmMessages) ban.user.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından banınız kaldırıldı!`}).catch(() => {});
