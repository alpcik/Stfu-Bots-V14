...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    {
    message.react("❌")
    message.channel.send({ content:"Yeterli yetkin bulunmuyor stfu ile görüş !"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 }).exec(async (err, res) => {
        ...
        let stfuingo = table(xd.slice(0, 15), config)
        let stfu = table(xd2, config)
    ...
    if (button.customId === "dosya") {
      ...
      button.reply({ content: `${member} kullanıcısının toplamda **${cezaData ? cezaData.ceza.length : 0}** cezası  bulunmaktadır ve aşağıdaki belgede yazmaktadır.`, files: [{ attachment: Buffer.from(stfu), name: `${member.user.username}_sicil.txt` }], components: [], ephemeral: true }); 
