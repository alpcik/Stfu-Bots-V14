...
await button.reply({ embeds: [stfu], components : [], ephemeral: true });
}
...
Sunucumuza **1 saat** içerisinde ${rakam(message.guild.members.cache.filter(stfu => (new Date().getTime() - stfu.joinedTimestamp) < 1000 * 60 * 60).size)} kullanıcı giriş yapmış.
Sunucumuza **1 gün** içerisinde ${rakam(message.guild.members.cache.filter(stfu => (new Date().getTime() - stfu.joinedTimestamp) < 1000 * 60 * 60 * 24).size)} kullanıcı giriş yapmış.
Sunucumuza **1 hafta** içerisinde ${rakam(message.guild.members.cache.filter(stfu => (new Date().getTime() - stfu.joinedTimestamp) < 1000 * 60 * 60 * 24 * 7).size)} kullanıcı giriş yapmış.
Sunucumuza **1 ay** içerisinde ${rakam(message.guild.members.cache.filter(stfu => (new Date().getTime() - stfu.joinedTimestamp) < 1000 * 60 * 60 * 24 * 30).size)} kullanıcı giriş yapmış.
`)
.setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
