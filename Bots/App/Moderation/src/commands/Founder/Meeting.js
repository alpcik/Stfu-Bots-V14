...
const { star, red, green } = require("../../../../src/configs/emojis.json");
...
embed.addFields({ name: `:star2: Son Toplantı Bilgisi`, value: `
> Toplantıya **\`${MeetingData.Joining.length}\`** yetkili katılmış :white_check_mark:
> Toplantıya **\`${MeetingData.Leaving.length}\`** yetkili katılmamış :x:
...
**${guild.name}** sunucusunda şuan da toplantı durumu: **${meetingStatus ? "Aktif" : "Devre-dışı"}** ${meetingStatus ? green : red}`)
        ...
        collector.on('collect', async (i) => {
          if(i.customId == "toplantıAç") {
              let meetingStatus = MeetingData.Toplantı || false
              if(meetingStatus) {
                await meetings.updateOne({guildID: stfucum.GuildID}, {$set: {Toplantı: false}}, {upsert: true})
...
:star2: **Bitirilen Toplantı Bilgisi**
> Bu toplantıda katılması beklenen **(\`${KatilimData.Leaving.length + KatilimData.Joining.length} yetkili\`)**
> Bu toplantıda katılım sağlayan: **(\`${KatilimData.Joining.length} yetkili\`)** :white_check_mark:
> Bu toplantıda katılım sağlamayan: **(\`${KatilimData.Leaving.length} yetkili\`)** :x:
> Bu toplantı  **\`${meetingTime(KatilimData.endDate - KatilimData.Date)}\`** sürmüş.`)], components: []})
                ...
                let datax = await meetings.findOne({ guildID: stfucum.GuildID })
                if(!datax) {await meetings.updateOne({guildID: stfucum.GuildID}, {$set: {Toplantı: true}}, {upsert: true})}
                await meetings.updateOne({guildID: message.guild.id}, {$set: {
                  "Date": Date.now(),
                  "userID": i.user.id,
                ...
                let datax2 = await meetings.findOne({ guildID: stfucum.GuildID })
                if (datax2) {
