    ...
    run: async (client, message, args, embed) => {
        if (!message.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) return;
          if (args[0] && args[0] < 101 && args[0] > 0 && !isNaN(args[0])) {
            ...
            message.channel.send({ content: `:white_check_mark: ${args[0]} adet mesaj silindi.` }).then((e) => setTimeout(() => { e.delete(); }, 5000));
        ...
        `)
            ...
            collector.on("collect", async (button) => {
              ...
              if (button.customId === "on") {
                ...
                message.channel.send({ content: `:white_check_mark: 10 adet mesaj silindi!` }).then((e) => setTimeout(() => { e.delete(); }, 5000));
              }
              if (button.customId === "yirmibes") {
                await message.delete();
                await message.channel.bulkDelete(25);
                message.channel.send({ content: `:white_check_mark: 25 adet mesaj silindi!` }).then((e) => setTimeout(() => { e.delete(); }, 5000));
              }
              if (button.customId === "elli") {
                await message.delete();
                await message.channel.bulkDelete(50);
                message.channel.send({ content: `:white_check_mark: 50 adet mesaj silindi!` }).then((e) => setTimeout(() => { e.delete(); }, 5000));
              }
              if (button.customId === "yüz") {
                await message.delete();
                await message.channel.bulkDelete(99);
                message.channel.send({ content: `:white_check_mark: 100 adet mesaj silindi!` }).then((e) => setTimeout(() => { e.delete(); }, 5000));
              }
              if (button.customId === "iptal") {
                await message.delete();
                msg.edit({ content: `:x: Mesaj silme işleminden vazgeçtiniz.`, embeds: [], components: [] }).then((e) => setTimeout(() => { e.delete(); }, 5000));
              }
            })
          }
        },
