...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.reply({ content: `Kanıt İçin Bir Ekran Görüntüsü Atınız! (*30 saniye sonra işlem iptal edilecektir.*)`})
    .then(() => {
      message.channel.awaitMessages({
        ...
        message.reply({ content :`:white_check_mark: ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``});
        if (stfucum.Bots.dmMessages) member.send({ content :`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle jaillendiniz.`}).catch(() => {});
        ...
        }).catch(err => message.reply({ content :`:x: 2 dakika içerisinde kanıt ekran görüntüsü atılmadığı için iptal edildi.`}));
      });
