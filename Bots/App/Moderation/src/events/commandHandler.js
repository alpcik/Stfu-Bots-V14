...
module.exports = async (message) => {
  let prefix = stfucum.Bots.prefix.find((x) => message.content.toLowerCase().startsWith(x));
  ...
  const embed = new EmbedBuilder().setFooter({ text: stfucum.Footer}).setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true })});
  ...
  if (cmd) {
    if (cmd.conf.owner && !stfucum.StfuID.includes(message.author.id)) return;
    ...
    `)
    if(komutLog) komutLog.wsend({ embeds: [stfu]})
  }
};
