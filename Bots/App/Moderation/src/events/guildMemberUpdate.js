    ...
    newMember.roles.cache.forEach(async role => {
      if (!oldMember.roles.cache.has(role.id)) {
        const emed = new EmbedBuilder()
          ...
          .setDescription(`Kişinin eklenen ve alınan tüm rollerine bakmak için \`!rollog @stfu\` komutunu kullanın \n**Rol Eklenen kişi**\n ${hedef} - **${hedef.id}** `)
          .addFields(
            { name: `:white_check_mark: Rolü Ekleyen Kişi`, value: `${yapan} - **${yapan.id}**`, inline: false },
            { name: `:white_check_mark: Eklenen Rol`, value: `${role} - **${role.id}**`, inline: false }
          )
    ...
    oldMember.roles.cache.forEach(async role => {
      if (!newMember.roles.cache.has(role.id)) {
        ...
        .setAuthor({ name: hedef.tag, iconURL: hedef.displayAvatarURL({ dynamic: true }) })
          ...
          .setDescription(`Kişinin alınan ve eklenen tüm rollerine bakmak için \`!rollog @stfu\` komutunu kullanın \n**Rolü Alınan kişi** \n${hedef} - **${hedef.id}**`)
          .addFields(
            { name: `:x: Rolü Alan Kişi`, value: `${yapan} - **${yapan.id}**`, inline: false },
            { name: `:x: Alınan Rol`, value: `${role} - **${role.id}**`, inline: false }
          )
          .setFooter({ text: yapan.tag, iconURL: yapan.displayAvatarURL({ dynamic: true }) })
          .setTimestamp()
