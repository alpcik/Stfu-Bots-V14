...
module.exports = async (message) => {
    ...
    yazilanKomut = yazilanKomut.slice(stfucum.Bots.prefix.some(x => x.length));
    ...
    if (message.member.roles.cache.some(rol => komut.YetkiliRol.includes(rol.id)) || message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        if (!üye) return message.reply({ embeds: [new EmbedBuilder().setDescription(`:x: Lütfen rol verilecek kişiyi etiketle.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        if (!komut.verilecekRol.some(rol => üye.roles.cache.has(rol))) {
            üye.roles.add(komut.verilecekRol)
            message.react("✅")
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`:white_check_mark: Başarılı şekilde ${üye} kişisine ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü verdim.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
            ...
            message.react("✅")
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`:white_check_mark: Başarılı şekilde ${üye} kişisinden ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü aldım.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
