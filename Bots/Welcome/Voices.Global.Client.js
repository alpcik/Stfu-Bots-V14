...
class stfu extends Client {
    ...
    async _start(channelId, a) {
        ...
        let guild = this.guilds.cache.get(stfucum.GuildID);
        if(!guild) return;
...
VoiceChannel.prototype.hasStaff = function(checkMember = false) {
    if(this.members.some(m => (checkMember !== false ? m.user.id !== checkMember.id : true) && !m.user.bot && m.roles.highest.position >= m.guild.roles.cache.get('1090742572495941672').position)) return true;
    return false;
}

VoiceChannel.prototype.getStaffs = function(checkMember = false) {
    return this.members.filter(m => (checkMember !== false ? m.user.id !== checkMember.id : true) && !m.user.bot && m.roles.highest.position >= m.guild.roles.cache.get('1090742572495941672').position).size
}
